...
export function Dashboard({ isSidebarOpen, onSidebarClose, onNavigate, onOpenNotes }: DashboardProps) {
  ...
  // Filter for personal dashboard: projects assigned to the user, or created by the user and not assigned to anyone else
  const personalProjects = projects.filter(p => 
    p.assignedTo === user?.uid || (!p.assignedTo && p.userId === user?.uid)
  );
