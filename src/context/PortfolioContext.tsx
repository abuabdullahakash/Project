...
export type WPPlugin = PortfolioData['plugins'][0];
export type Testimonial = PortfolioData['testimonials'][0];
...
interface PortfolioContextType {
  ...
  addPlugin: (plugin: WPPlugin) => void;
  updatePlugin: (id: string, plugin: WPPlugin) => void;
...
export function PortfolioProvider({ children, initialEditMode = false }: { children: React.ReactNode, initialEditMode?: boolean }) {
      ...
      if (saved) {
        try {
          ...
          const updatedData = { 
            ...
            plugins: parsedData.plugins || initialData.plugins,
            testimonials: parsedData.testimonials || initialData.testimonials
          };
          setData(updatedData);
        ...
        if (urlPortfolioId) {
          ...
          unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
              ...
              setData({
                ...initialData,
                ...currentData,
                plugins: currentData.plugins || initialData.plugins,
                testimonials: currentData.testimonials || initialData.testimonials
              });
            } else {
              setData(initialData);
            }
          ...
          unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
              ...
              if (JSON.stringify(currentData.personalInfo) !== JSON.stringify(initialData.personalInfo) || !currentData.plugins || !currentData.testimonials) {
          ...
          if (!querySnapshot.empty) {
            ...
            unsubscribe = onSnapshot(docSnap.ref, (snap) => {
                if (snap.exists()) {
                  const currentData = snap.data() as PortfolioData;
                  setData({
                    ...initialData,
                    ...currentData,
                    plugins: currentData.plugins || initialData.plugins,
  ...
  const addPlugin = (plugin: WPPlugin) => {
    setData(prev => ({ ...prev, plugins: [...(prev.plugins || []), plugin] }));
  };

  const updatePlugin = (id: string, plugin: WPPlugin) => {
    setData(prev => ({
      ...prev,
      plugins: (prev.plugins || []).map(p => p.id === id ? plugin : p)
    }));
  };
  ...
  const updateTestimonial = (id: string, testimonial: Testimonial) => {
    setData(prev => ({
      ...prev,
  ...
  return (
    <PortfolioContext.Provider value={{
      ...
      saveChanges, isLoading, portfolioId
    }}>
      {children}
