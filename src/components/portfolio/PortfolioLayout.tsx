    ...
    <nav className="fixed top-0 w-full z-50 bg-[#050505]/80 backdrop-blur-md border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-300">
          ...
          <button onClick={() => onNavigate('/portfolio/plugins')} className="hover:text-white transition-colors"><EditableText value={personalInfo.navPlugins || 'Plugins'} onSave={(val) => updatePersonalInfo('navPlugins', val)} /></button>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#050505] border-b border-white/5 px-6 py-4 flex flex-col gap-4 absolute w-full left-0 top-20 shadow-2xl">
          ...
          <button onClick={() => { onNavigate('/portfolio/plugins'); setIsMobileMenuOpen(false); }} className="text-left text-slate-300 hover:text-white py-2 font-medium"><EditableText value={personalInfo.navPlugins || 'Plugins'} onSave={(val) => updatePersonalInfo('navPlugins', val)} /></button>
